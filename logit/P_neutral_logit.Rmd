...
It shows that the price has 63 missing value. There two potential reasons. One is that the product is not available at that quarter. Another the price data is not recorded at that quarter.
...
According to the coefficient of Averate2, an increase of one average star in rating will result in an increase of 2.509 unit of market share. This coefficient is too large. According to the table, a one-unit rise in price (dollars) will result in a 1% drop in market share.
$$
...
for (i in c('R20Q4','R21Q1','R21Q2','R21Q3','R21Q4','R22Q1')) {
  ...
  FOC <- function(innitial_vetor){
    ...
    prices <- price_pre# In p-neutral logit, the price will not change
    ...
    price_mc <- prices - marg_cost+ delta# We let the product efficiency change in this model here
  ...
  library(BB)
  innitial_vector <- append(rep(0,length(data.R22Q1$Averate2)),data.R22Q1$Averate2)# The innitial value for the product efficiency need to find a proper one
  ...
  EstqualityPre.pg[which(EstqualityPre.pg<0,arr.ind = T)] <- 0.5
  ...
  EstqualityPost.pg[which(EstqualityPost.pg<0,arr.ind = T)] <- 1
