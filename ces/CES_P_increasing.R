...
shareshow <- datatest2 %>% group_by(Firm) %>%
  summarise(Mshare = mean(shares)) %>%
  ungroup()
shareshow<-shareshow[order(shareshow$Mshare,decreasing = T),]

stargazer(shareshow[c(1:10),],summary = F)
