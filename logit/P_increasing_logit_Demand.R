...
linear.2 <- feols(mc_quality_pre ~ Averate| Brand , datatest5)
...
for (i in market_list) {
  ...
  #1. tHE DEFAULT DFSANCE WITH DEFAULT DOES NOT WORK, nOW i WILL TRY THE 
  ...
  # minResult.pre <- BBsolve(innitial_vector,FOC,method=1, control=list(M=50))
  # minResult.pre <- dfsane(innitial_vector,FOC,quiet=FALSE,control=list(maxit = 2000))
  ##Use BBoptim to find the optimization value haha
  ...
  #   lo <- c(rep(0,len/2),rep(0,len/2))  
  #   hi <- c(rep(1500,len/2),rep(5,len/2))
  #   minResult.pre <- BBoptim(innitial_vector,FOC_optim,lower=lo, upper=hi)
  # }
  ...
  FOC_post <- function(innitial_vetor){
    innitial_vetor <- innitial_vector
    len <- length(innitial_vetor)
    prices <- innitial_vetor[1:(len/2)]
    quality <- innitial_vetor[(len/2+1):len]
    implied_shares <- calcShares(prices,quality,meanval,alpha,tau)
...
saveWorkbook(wb1, "logit_p_increasing3.xlsx", overwrite = TRUE)


# tt <- read.xlsx('./RESULT.xlsx',sheet = 'UNIT',detectDates = TRUE,fillMergedCells = TRUE)
# library(xtable)
