...
# write.csv(Sum_dataframe,'Sum_dataframeces.csv')
...
for (i in seq(1,length(b[,1]),1)) {
  ...
  tryCatch({
    m_iv <- ivreg(formula, data = test[,c(1:10,b[i,1],b[i,2],b[i,3],b[i,4],27:122)])
    summ_m_iv <- summary(m_iv,diagnostics = TRUE)
  },
  error=function(e) {
    # message(paste("URL does not seem to exist:", url))
  ...
  fs_m_iv_endo1 <- lm(formula_lh_endo_1, data = test[,c(1:10,b[i,1],b[i,2],b[i,3],b[i,4],27:122)])
  fs_m_iv_endo2 <- lm(formula_lh_endo_2, data = test[,c(1:10,b[i,1],b[i,2],b[i,3],b[i,4],27:122)])
  ...
  if (m_iv$coefficients[3]<0 & m_iv$coefficients[2]>0 ){
    ...
    right <- rbind(right,c(i,testnames[b[i,1]],testnames[b[i,2]],testnames[b[i,3]],testnames[b[i,4]],coeff[2,1],coeff[3,1],coeff[2,4],coeff[3,4],diag[1,4],
                           diag[2,4],diag[3,4],diag[4,4],lh_m_iv_endo1$F[2], lh_m_iv_endo2$F[2], score))
    cat(c(i,testnames[b[i,1]],testnames[b[i,2]],testnames[b[i,3]],testnames[b[i,4]],coeff[2,1],coeff[3,1],coeff[2,4],coeff[3,4],diag[1,4],
          diag[2,4],diag[3,4],diag[4,4],lh_m_iv_endo1$F[2], lh_m_iv_endo2$F[2], score))
    #stargazer(m_iv,omit =name,type = 'text')
...
colnames(Sum_dataframe_4iv) <- c('b','iv1','iv2','iv3','iv4','quality_coef','price_coef','quality_coef_Pvalue','price_coef_Pvalue',
                                 'WeakIV_1','WeakIV_2','WuHausman','Sargan','iv_endo1_Ftest','iv_endo1_Ftes2','score')
...
# merger simulation -------------------------------------------------------
i <- 102 #2,3,5,7,8,9,14,18
formula <- as.formula(sprintf('lgs.s0 ~.-%s -%s -%s | .-%s -%s',testnames[b[i,1]],testnames[b[i,2]],testnames[b[i,3]],'Prices','Averate'))
m_iv_put <- ivreg(formula, data = test[,c(1:9,b[i,1],b[i,2],b[i,3],24:122)])
...
#ols regression
ols <- lm('lgs.s0 ~.',data = test[,c(1:9,24:122)])

stargazer(ols,fs_m_iv_endo1,fs_m_iv_endo2,m_iv_put,olsR,fs_m_iv_endo1R,fs_m_iv_endo2R,m_iv_putR,omit =name)


# alpha <- -0.07065  # get from the two step gmm result. Coefficient of price
# tau <- 6.31321# Coefficient of quality g
alpha <- m_iv_put$coefficients[3]   # get from the two step gmm result. Coefficient of price
tau <- m_iv_put$coefficients[2]# Coefficient of quality g
...
for (i in market_list) {
  ...
  # lo <- c(rep(-1000,len/2),rep(0,len/2))
  # hi <- c(rep(1000,len/2),rep(5,len/2))
  ...
  # EstqualityPre.pg[which(EstqualityPre.pg<0,arr.ind = T)] 
  # EstqualityPre.pg[which(EstqualityPre.pg>5,arr.ind = T)] 
  ...
  # EstqualityPost.pg[which(EstqualityPost.pg<0,arr.ind = T)] 
  # EstqualityPost.pg[which(EstqualityPost.pg>5,arr.ind = T)] 
  ...
  Est_result.pg$deltaDif.pg <- Est_result.pg$EstdeltaPost.pg - Est_result.pg$EstdeltaPre.pg
  ...
  Pro1$EST_ob_sharepg <- Pro1$EstsharesPre.pg-Pro1$Ob.shares
...
saveWorkbook(wb1, "logit_p_neutral.xlsx", overwrite = TRUE)
