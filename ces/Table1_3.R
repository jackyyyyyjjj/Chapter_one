...
table <- read.xlsx('ces_d.xlsx',sheet = 'sum')
...
sum <- as.data.frame(summary(table_other[,-c(1:3)]))
stargazer(table_other[,-c(1:3)],digits = 3)
...
table <- read.xlsx('ces_p_neutral.xlsx',sheet = 'sum')
# table <- table[table$converge_pre==0,]
table_merge <- table[table$label=='*',]
table_merge <- table_merge[complete.cases(table$label),]
table_other <- table[!complete.cases(table$label),]
...
# table_neut <- read.xlsx('ces_d_delta.xlsx',sheet = 'R22Q1')
#Proposition 1
...
data_pneu <- table_merge[,c(1,12:15)]
k <- data_pneu
k1 <- mapply(as.numeric, k)

k1 <- as.data.frame(k1)
k1$brand_pre <- data_pneu$brand_pre
...
stargazer(sumtab2,summary = FALSE,digits = 3)
