...
# #season fixed effect
# dummycol_time <- data.frame(str_extract(datatest$market_ids,'\\d$'))
# colnames(dummycol_time) <- c("quarter")
# library(fastDummies)
# test.df_time <- dummy_cols(dummycol_time, select_columns = "quarter", remove_first_dummy = T)
# test.df_time2 <- test.df_time[,-1]
# datatest2 <- cbind(datatest2, test.df_time2)
# 
# 
# # time trend fixed effect
# dummycol_time <- data.frame(as.numeric(factor(datatest$market_ids, levels= unique(datatest$market_ids))))
# colnames(dummycol_time) <- c("trend_quarter")
# library(fastDummies)
# test.df_time <- dummy_cols(dummycol_time, select_columns = "trend_quarter", remove_first_dummy = T)
# test.df_time3 <- test.df_time[,-1]
# datatest2 <- cbind(datatest2, test.df_time3)



#First stage  12:157,158:173,174:176,177:192
# stage1_price <- lm(prices~ .-shares -Averate2,data = test2[,-c(6,10,174:176,177:192)])
# summary(stage1_price)
# 
# stage1_quality <- lm(Averate2~ .-prices -shares,data = test2[,-c(6,10,174:176,177:192)])
...
for (i in c('R20Q4','R21Q1','R21Q2','R21Q3','R21Q4','R22Q1')) {
  ...
  FOC_p.pre <- function(innitial_vetor){
    ...
    # markups_post <- c(-solve(t(derivatives_price) * ownerpre) %*% implied_shares)
    marg_cost <- calcMc_price.p(price_pre,meanval,quality,alpha,tau,ownerpre,B)
    ...
    # matrix_shares_mC <- matrix(matrix_shares_mC,ncol = 1)
    thisFOC <- as.vector(implied_shares*B/prices + M1_PRICE %*% price_mc)
    return(thisFOC)
  }
...
saveWorkbook(wb1, "ces_p_increasing.xlsx", overwrite = TRUE)
